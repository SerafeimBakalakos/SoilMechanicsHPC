...
namespace ISAAR.MSolve.Analyzers.NonLinear
{
    public class LoadControlAnalyzer : NonLinearAnalyzerBase
    {
        ...
        public double dispfail2 = 0.0;
        public double rotfail = 0.0;
            ...
            for (int increment = 0; increment < numIncrements; increment++)
            {
               ...
               // Debug.WriteLine("NR {0}, first error: {1}, exit error: {2}", iteration, firstError, errorNorm);
                ...
                if (model.Subdomains[0].hasfailed == true)
                {
                    ...
                    dispfail = (uPlusdu[1][10] + uPlusdu[1][573] + uPlusdu[1][577] + uPlusdu[1][589]) / 4.0;
                    dispfail2 = (uPlusdu[1][8] + uPlusdu[1][571] + uPlusdu[1][575] + uPlusdu[1][587]) / 4.0;
                    rotfail = (uPlusdu[1][10] - uPlusdu[1][577] - uPlusdu[1][589] + uPlusdu[1][573]) / 1.0;
